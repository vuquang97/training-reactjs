...
class User extends Component {
  ...
  componentWillMount() {
    if (localStorage && localStorage.getItem('USER')) {
      var task = JSON.parse(localStorage.getItem('USER')); // lấy dữ liệu ra và chuyển thành kiểu object
      this.setState({
        task: task
      });
    }
  ...
  submitTaskForm = (data) => {
    ...
    localStorage.setItem('USER', JSON.stringify(task)) //lưu dữ liệu vào local storage
  }
  ...
  render() {
    ...
    if (filter) {
      if (filter.name) {
        task = task.filter((task) => {
          return task.txtUsername.toLowerCase().indexOf(filter.name) !== -1;  //tìm kiếm theo name
        });
      }
      task = task.filter((task) => {
        if (filter.status === -1) {
    ...
    if(sortBy === 'name') {   /// sắp xếp
      task.sort((a,b) => {
        if(a.txtUsername < b.txtUsername) return -sortValue;
        else if(a.txtUsername > b.txtUsername) return sortValue;
        else return 0;
      });
    }else {
      task.sort((a, b) => {
    ...
    return (
      <div className="container">
        <Link to='/admin'><button type="button" className="btn btn-warning"><i className="fas fa-arrow-left"></i>&nbsp;Quay lại</button></Link>
        <Link to='/'><button type="button" className="btn btn-warning"><i className="fas fa-home"></i>&nbsp;Home</button></Link>
        <div className="text-center">
          <h1>Quản Lý User</h1><hr /><br />
        </div>
        <div className="row">
          ...
          <div className={isDisplayForm ? "col-xs-8 col-sm-8 col-md-8 col-lg-8" : "col-xs-12 col-sm-12 col-md-12 col-lg-12"}>
            <div className="row">
              <div className="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                <button className="btn btn-primary" type="button" onClick={this.handleToggleForm}><span className="fa fa-plus"></span>&nbsp;Thêm User</button>
              </div>
              <div className="col-xs-6 col-sm-6 col-md-6 col-lg-6">
